...
- 🎨 **模板制作向导**：可视化流程，轻松制作和编辑 PPT 模板
- 🔧 **模板编辑器**：在线编辑 PPT 元素，AI 一键命名/查漏补缺，批量操作
- 📊 **已发布模板管理**：查看、编辑、重新发布已有模板
...
1. **上传 PPT**：上传 PPTX 文件，自动解析页面和元素，生成预览图
2. **编辑命名**：为每个元素设置语义化名称、提示信息、字数限制（支持 AI 一键命名/查漏补缺）
3. **发布模板**：保存到模板库，可直接用于 PPT 生成
...
- 🖼️ 可视化预览，元素高亮标注
- ✏️ 在线编辑元素名称、提示信息、字数限制、必填项
- 🤖 **AI 一键命名**：强制重新命名所有页面元素
...
- ✨ **已发布模板管理**
  ...
  - 编辑后可保存覆盖原模板
- ✨ **AI 命名增强**
  - 新增「AI 查漏补缺」模式：只补全缺失配置，保留已有命名
  - 优化字数限制估算：更保守的预估，PPT 文字宜精简
  - 顶栏双按钮：「AI 一键命名」和「AI 查漏补缺」
- 🐛 **问题修复**
  - 修复 PPT 形状名称未更新到文件的问题（shape_id 匹配）
  - 修复图文页无图片时占位符被删除的问题
  - 修复编辑已发布模板时预览图不显示的问题
  - 发布模板后自动清理临时会话和文件
...
- ✨ **模板制作向导**
  - 三步向导流程：上传 PPT → 编辑命名 → 发布
  - 支持 iframe 嵌入编辑器，统一的向导体验
  - 会话保存和恢复，随时继续编辑
- ✨ **PPT 模板编辑器**
  - AI 一键命名全部页（支持向导模式）
  - 元素配置：提示信息、字数限制、必填项、页面备注
  - 批量操作：多选、隐藏、显示元素
- ✨ **编辑会话管理**
  - 自动保存编辑进度
  - 编辑记录列表，一键恢复
